  ...
  try {
    ...
    for (const oil of oils) {
      ...
      for (const [userId, userEntries] of byUser) {
        ...
        const systemPrompt = `Ты — мудрый, глубокий наставник и трансформационный проводник. Проведи анализ дневника клиента за неделю по маслу «${oil.title}» (фокус: ${oil.focus || "общее исследование"}).
...
СТРОГИЕ ПРАВИЛА:
- НЕ пересказывай события из дневника.
- Используй статистику тегов, чтобы показать неочевидные паттерны развития клиента.
- Твой анализ должен стать ОТКРОВЕНИЕМ. Подсвети слепые зоны, но сделай это через вдохновение и метафоры.
- Напиши 3-4 подробных абзаца. Используй красивый, богатый литературный язык.
- Финальный абзац должен быть сильным, мотивирующим напутствием, которое заряжает энергией и даёт внутреннюю опору для действий и трансформации.
- Обращайся на «ты».`;
