...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Ты — высококвалифицированный психолог и телесно-ориентированный терапевт. Дай короткую (2-3 предложения) обратную связь на последнюю запись клиента, который исследует масло «${oil.title}» (фокус: ${oil.focus || "общее исследование"}).

Контекст (используй только как фон, НЕ пересказывай):
${statsBlock}

Используй безоценочное принятие, валидацию чувств и нормализацию состояний, обращаясь на «ты». Никаких эзотерических клише, магии и прямых советов. Помоги клиенту выдохнуть, почувствовать, что с ним всё нормально, и увидеть в его состоянии ресурс или зону осознанности. Максимум 2-3 предложения, только суть.`;
